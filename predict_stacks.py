...
def tile_iterator(im,
    ...
    for i, index in enumerate(product(*[range(sg) for sg in subgrids])):
        ...
        s_output = tuple([slice(p, -p-pm*(i == s-1))
                          for pm, p, i, s in zip(pad_mismatch, padsize, index, subgrids)])

        s_output = tuple([slice(p, b+p-pm*(i == s-1))
...
if __name__ == '__main__':
    ...
    for i, f in enumerate(files):
        ...
        if (args.datatype == "MDA231"):
            model_center = load_model("models/MDA231_center_cell.h5",
                                      custom_objects={'dice_loss': dice_loss, 'dice_coef': dice_coef})
            model_mask = load_model("models/MDA_231_prob_cell.h5",
                                    custom_objects={'dice_loss': dice_loss, 'dice_coef': dice_coef})
            img = imread(f)
            ...
            peaks = peak_local_max(center, min_distance=args.min_distance,
                                   threshold_abs=args.threshold_abs, indices=False, exclude_border=1)
        else:
            ...
            u_mask = apply(model_mask, np.pad(x, ((2, 2), (0, 0), (0, 0)),
                                              mode="reflect"), tile_size=args.tile_size)[2:-2]
            peaks = peak_local_max(u_center, min_distance=args.min_distance,
                                   threshold_abs=args.threshold_abs, indices=False, exclude_border=1)
